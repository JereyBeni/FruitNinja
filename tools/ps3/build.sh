...
#
# Usage (from a shell that has PS3DEV + PSL1GHT in the environment):
...
if [ -z "${PS3DEV:-}" ] || [ ! -x "${PS3DEV}/ppu/bin/ppu-g++" ]; then
    ...
    echo "Install ps3toolchain + PSL1GHT and export:" >&2
...
echo
echo "Scaffold build finished."
echo "Binary (when the port is further along) will land under $BUILD_DIR"
echo "Next steps: wire mainPS3.cpp + RSX + pad, then package .self/.pkg"
